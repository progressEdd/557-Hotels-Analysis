...
multi5_plot = ggplot(data = multi5, mapping = aes(x = arrival_date_month, y = adr, fill = hotel)) + xlab("Month") + ylab("Average ADR (Rounded to Nearest ADR)") + geom_col()+ geom_text(aes(label=adr), position = position_stack(vjust= 0.5),
            colour = "black", size = 5) + coord_flip()
